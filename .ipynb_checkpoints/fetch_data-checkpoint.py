...
Data sources:
  ...
  3. Fama-French 5 factors + Mom    -- Ken French data library via pandas-datareader
...
MOM_RAW_PATH = os.path.join(DATA_DIR, "raw_mom.csv")
...
MANUAL_DOWNLOAD_MSG = """
  ...
  │  2. Go to:                                                      │
  │     {mom_url}                                                   │
  │     → Save the CSV inside the ZIP as:  data/raw_mom.csv        │
  │                                                                 │
  │  Then re-run: python fetch_data.py                              │
  └─────────────────────────────────────────────────────────────────┘
""".format(ff5_url=FF5_DOWNLOAD_URL, mom_url=MOM_DOWNLOAD_URL)
...
def fetch_ff_factors() -> pd.DataFrame:
    """
    Load Fama-French 5 factors + Momentum from manually downloaded CSVs.
    ...
    if not os.path.exists(MOM_RAW_PATH):
        missing.append("data/raw_mom.csv")

    if missing:
        print(f"\n  Missing raw factor files: {missing}")
        print(MANUAL_DOWNLOAD_MSG)
        raise FileNotFoundError(
            "Please download the FF factor CSVs manually (see instructions above)."
        )

    print("  Parsing FF5 factors...")
    df5 = _parse_french_csv(FF5_RAW_PATH)
    # Standardize column names (French uses 'Mkt-RF', 'SMB', 'HML', 'RMW', 'CMA', 'RF')
    df5.columns = [c.strip() for c in df5.columns]

    print("  Parsing Momentum factor...")
    dfm = _parse_french_csv(MOM_RAW_PATH)
    dfm.columns = [c.strip() for c in dfm.columns]
    # Momentum column is named 'Mom' in French's file
    mom_col = [c for c in dfm.columns if "mom" in c.lower() or "wml" in c.lower()]
    if mom_col:
        dfm = dfm[[mom_col[0]]].rename(columns={mom_col[0]: "Mom"})

    factors = df5.join(dfm, how="left")
